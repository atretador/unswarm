...
public sealed class SchedulerWorkerContainerAwareTests : IDisposable
{
    ...
    [Fact]
    public async Task NoContainerRegistry_FailsRequestsFast()
    {
        // Without a container registry the scheduler cannot route models to
        // registered runtimes: requests must fail immediately, touching no docker.
        var channel = Channel.CreateUnbounded<InferenceRequest>();
        ...
        var r2 = MakeRequest("model-b", id: "r2");

        await channel.Writer.WriteAsync(r1);
        await channel.Writer.WriteAsync(r2);

        var ex1 = await Assert.ThrowsAsync<InvalidOperationException>(() => r1.Tcs.Task.WaitAsync(TimeSpan.FromSeconds(5)));
        Assert.Contains("No container registry configured", ex1.Message);
        var ex2 = await Assert.ThrowsAsync<InvalidOperationException>(() => r2.Tcs.Task.WaitAsync(TimeSpan.FromSeconds(5)));
        Assert.Contains("No container registry configured", ex2.Message);

        // Nothing was started or stopped — routing failed before any container work.
        Assert.Empty(_docker.StartedModels);
        Assert.Empty(_docker.StoppedContainerIds);
